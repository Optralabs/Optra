...
from globals import get_user_plan
from auth import verify_token, create_token
from globals import set_auth_cookie, log_session
...
st.markdown("""
<style>
 .block-container {max-width: 1200px; padding-top: 6rem !important; padding-bottom: 0.5rem;}
 ...
 .optra-header { display:flex; align-items:center; margin-bottom: 16px; }
 .optra-header img { margin-right: 12px; }
 .desc { color:#c7cfdd; font-size: 0.96rem; margin: 12px 0 16px 0; }
 .hr { margin: 12px 0 18px 0; border:none; border-top:1px solid #2b3a5e; }
 .help { color:#b7c1d1; font-size:0.9rem; margin-top:10px; }
 ...
 /* Buttons same height, auto width */
 .stButton > button {
     height: 44px; padding: 0 16px; border-radius: 10px;
     border: 1px solid #2b3a5e; background: #2c344d; color: #fff; font-weight: 600;
     width: auto;
 }


 .tight-gap > div { padding-right: 0 !important; }
</style>
""", unsafe_allow_html=True)


# ===== Logo + Title =====
def _logo_b64(path="optra_logo_transparent.png", width=72):
   from PIL import Image; from io import BytesIO; import base64
   img = Image.open(path); img = img.resize((width, width), Image.Resampling.LANCZOS)
...
st.markdown(
   f"""
   <div class='optra-header'>
       <img src='data:image/png;base64,{_logo_b64()}' width='72'/>
       <h1 style='margin:0; font-size:1.9rem; color:white;'>OPTRA</h1>
   </div>
   """,
   unsafe_allow_html=True
)
...
# ===== Short description =====
st.markdown(
   "<div class='desc'>Welcome to the Smart Grant Advisor. Review documents, check eligibility, plan timelines and generate grant emails in one place. Access depends on your plan.</div>",
   unsafe_allow_html=True,
)
...
# ===== Email input =====
email = st.text_input("Email", placeholder="you@company.com", label_visibility="visible")


# ===== Buttons Row =====
col_unlock, col_flex = st.columns([0.22, 0.78], gap="small")
with col_unlock:
   unlock_clicked = st.button("Unlock Access", key="unlock_btn")
   st.markdown(
       "<div style='font-size:12px; color: #999999; margin-top:6px;'>"
       "Validate your email and unlock features"
       "</div>",
       unsafe_allow_html=True
   )
...
def _auto_clear_login():
   ...
   for k in list(st.session_state.keys()):
       if k.startswith("auth_") or k in ("user_email", "user_plan", "unlocked"):
...
if unlock_clicked:
   ...
   if plan in ("Starter Plan", "Pro Plan"):
       ...
       # set token in URL (optional)
       try:
           st.query_params["token"] = create_token(norm_email)
       except Exception:
           pass


       # ✅ persist unlock for 24h and capture token
       token = set_auth_cookie(norm_email, plan)


       # ✅ log token hash + expiry to Supabase
       log_session(norm_email, token=token)


       st.success("Access unlocked")
       st.markdown(
           f"<span style='color:#ccc;'>Plan:</span> "
           f"<span class='pill'>{plan}</span>",
           unsafe_allow_html=True
       )
       st.rerun()
   else:
       st.error("Could not unlock access: account not found or no active plan.")


# ===== Access Gate =====
decoded = verify_token()
email_from_token = (decoded or {}).get("email")
plan_from_token = get_user_plan(email_from_token) if email_from_token else None
if not decoded or plan_from_token not in ("Starter Plan", "Pro Plan"):
   st.stop()
st.session_state["user_email"] = email_from_token
st.session_state["user_plan"]  = plan_from_token
st.session_state["unlocked"]   = True


# === Your Original Imports (DO NOT DELETE) ===
import streamlit as st
import openai
